...
export default function Home() {
  ...
  // Auto-pull the cached live fixtures for the selected competition once,
  // as soon as a signed-in user's workspace has finished loading — no
  // manual "Fetch live fixtures" click required. Always append-only: it
  // only ever adds fixtures that aren't already in the workspace, so it
  // can never overwrite advanced evidence, bet logs, or predictions a user
  // already entered on an existing fixture. The ref guard keeps this to
  // one attempt per page load rather than re-firing on every re-render
  // (e.g. React StrictMode's double-invoke in development).
  const hasAutoFetchedLiveFixtures = useRef(false);
  useEffect(() => {
    if (!session) return;
    if (!hasLoadedSavedState) return;
    if (hasAutoFetchedLiveFixtures.current) return;
    hasAutoFetchedLiveFixtures.current = true;
    loadLiveFixtures("append");
    // loadLiveFixtures intentionally omitted from deps: it closes over
    // `fixtures`/`liveFixturesCompetition` at call time, which is fine for
    // a one-shot fetch — re-adding it would re-run this on every fixture
    // change, defeating the "once per load" guard above.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [session, hasLoadedSavedState]);
