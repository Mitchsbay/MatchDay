...
// Recovered verbatim from the app's former bundled example data
// (lib/sampleData.ts), which now ships empty on purpose. These three
// fixtures exist only as realistic test inputs for this suite.
...
import {
  ...
  calculateLeagueGoalAverages,
  calculatePoissonOutcomeProbabilities,
  dixonColesTau,
  poissonPmf,
  FALLBACK_LEAGUE_GOAL_AVERAGES,
} from "../lib/expectedGoalsModel";
...
function runWeatherClientSmokeTests() {
  // Calm conditions -> no risk, no reasons.
  const calm = classifyWeatherRisk({ temperatureC: 18, windSpeedKph: 12, precipitationMm: 0 });
  assert.equal(calm.risk, false, "calm conditions should not flag risk");
  assert.equal(calm.reasons.length, 0, "calm conditions should have no reasons");

  // Each threshold should independently trigger risk.
  assert.equal(classifyWeatherRisk({ temperatureC: 18, windSpeedKph: 12, precipitationMm: 4 }).risk, true, "heavy precipitation should flag risk");
  assert.equal(classifyWeatherRisk({ temperatureC: 18, windSpeedKph: 31, precipitationMm: 0 }).risk, true, "wind above 30km/h should flag risk");
  assert.equal(classifyWeatherRisk({ temperatureC: 0, windSpeedKph: 12, precipitationMm: 0 }).risk, true, "sub-zero temperature should flag risk");
  assert.equal(classifyWeatherRisk({ temperatureC: 35, windSpeedKph: 12, precipitationMm: 0 }).risk, true, "extreme heat should flag risk");

  // Just under each threshold should not flag risk.
  assert.equal(classifyWeatherRisk({ temperatureC: 18, windSpeedKph: 30, precipitationMm: 3.9 }).risk, false, "borderline-safe values should not flag risk");

  // An "unavailable" assessment must never overwrite an existing manual flag.
  const unavailable: WeatherAssessment = {
    source: "unavailable",
    risk: false,
    reasons: [],
    snapshot: null,
  ...
  assert.equal(resolveWeatherDisruptionRisk(true, unavailable), true, "unavailable assessment must not clear an existing manual risk flag");
  assert.equal(resolveWeatherDisruptionRisk(undefined, unavailable), undefined, "unavailable assessment must not invent a flag where none existed");

  // A real assessment should override, in either direction.
  const stormy: WeatherAssessment = {
    source: "open-meteo",
    risk: true,
    reasons: ["High wind forecast (40km/h)."],
    snapshot: { temperatureC: 15, windSpeedKph: 40, precipitationMm: 0 },
    venue: { latitude: 0, longitude: 0, resolvedName: "Test City" },
    note: "test",
  };
  assert.equal(resolveWeatherDisruptionRisk(false, stormy), true, "a real assessment should override a stale manual flag");
}

runWeatherClientSmokeTests();

function runExpectedGoalsModelSmokeTests() {
  // --- poissonPmf ---
  // Probabilities across a wide enough range of k should sum to ~1.
  const pmfTotal = Array.from({ length: 20 }, (_, k) => poissonPmf(k, 2.5)).reduce((a, b) => a + b, 0);
  assert.ok(Math.abs(pmfTotal - 1) < 0.001, "Poisson pmf across k=0..19 should sum to ~1 for a reasonable lambda");
  assert.equal(poissonPmf(0, 0), 1, "zero expected goals should make zero goals certain");
  assert.equal(poissonPmf(1, 0), 0, "zero expected goals should make any goals impossible");

  // --- dixonColesTau ---
  // rho=0 must reduce to plain independent Poisson (tau === 1 everywhere).
  assert.equal(dixonColesTau(0, 0, 1.4, 1.1, 0), 1, "rho=0 should leave 0-0 unadjusted");
  assert.equal(dixonColesTau(2, 2, 1.4, 1.1, -0.1), 1, "tau should only adjust the four low-score cells");
  // With the standard negative rho, 0-0 and 1-1 draws should be boosted
  // relative to independent Poisson, matching the Dixon-Coles correction's
  // documented purpose.
  assert.ok(dixonColesTau(0, 0, 1.4, 1.1, -0.1) > 1, "negative rho should boost 0-0 probability");
  assert.ok(dixonColesTau(1, 1, 1.4, 1.1, -0.1) > 1, "negative rho should boost 1-1 probability");
  ...
  const leagueFixtures: Fixture[] = [
    {
      ...
      homeTeam: "Alpha",
      awayTeam: "Beta",
      homeStats: { ...emptyTeamStats, homePlayed: 10, homeGoalsFor: 20, homeGoalsAgainst: 8 },
      awayStats: { ...emptyTeamStats, awayPlayed: 10, awayGoalsFor: 8, awayGoalsAgainst: 15 },
    },
    {
      ...fixtures[0],
      id: "lg-2",
      ...
      homeTeam: "Gamma",
      awayTeam: "Delta",
      homeStats: { ...emptyTeamStats, homePlayed: 10, homeGoalsFor: 10, homeGoalsAgainst: 10 },
      awayStats: { ...emptyTeamStats, awayPlayed: 10, awayGoalsFor: 6, awayGoalsAgainst: 12 },
    },
  ];
  const leagueAverages = calculateLeagueGoalAverages(leagueFixtures, "Test League");
  assert.equal(leagueAverages.source, "league-data", "four distinct teams with played games should use real league data");
  assert.equal(leagueAverages.sampleTeams, 4, "should dedupe to exactly the four distinct teams across both fixtures");
  assert.equal(leagueAverages.avgHomeGoalsFor, 1.5, "(20/10 + 10/10) / 2 = 1.5 average home goals for");
  assert.equal(leagueAverages.avgAwayGoalsFor, 0.7, "(8/10 + 6/10) / 2 = 0.7 average away goals for");

  // A competition with no played games anywhere should fall back rather than divide by zero.
  const emptyLeague = calculateLeagueGoalAverages([{ ...fixtures[0], competition: "Empty League", homeStats: emptyTeamStats, awayStats: emptyTeamStats }], "Empty League");
  assert.equal(emptyLeague.source, "fallback", "a competition with zero played games should fall back to defaults");
  assert.equal(emptyLeague.avgHomeGoalsFor, FALLBACK_LEAGUE_GOAL_AVERAGES.avgHomeGoalsFor);

  // --- calculateExpectedGoals ---
  // A strong home attack against a weak away defense should clearly out-xG
  // a weak home attack against a strong away defense, all else equal.
  const strongHomeStats = { ...emptyTeamStats, homePlayed: 10, homeGoalsFor: 30, homeGoalsAgainst: 5 };
  const weakHomeStats = { ...emptyTeamStats, homePlayed: 10, homeGoalsFor: 5, homeGoalsAgainst: 20 };
  const weakAwayDefenseStats = { ...emptyTeamStats, awayPlayed: 10, awayGoalsFor: 10, awayGoalsAgainst: 25 };
  const strongAwayDefenseStats = { ...emptyTeamStats, awayPlayed: 10, awayGoalsAgainst: 5, awayGoalsFor: 10 };
  const league: import("../lib/expectedGoalsModel").LeagueGoalAverages = {
    avgHomeGoalsFor: 1.5,
    avgAwayGoalsFor: 1.15,
    sampleTeams: 20,
    source: "league-data",
  };
  const strongVsWeakDefense = calculateExpectedGoals(strongHomeStats, weakAwayDefenseStats, league);
  const weakVsStrongDefense = calculateExpectedGoals(weakHomeStats, strongAwayDefenseStats, league);
  assert.ok(
    strongVsWeakDefense.homeXg > weakVsStrongDefense.homeXg,
    "a strong attack facing a weak defense should have higher expected goals than a weak attack facing a strong defense",
  );

  // A team with zero games played should fall back to league-average
  // strength (1.0) rather than forcing expected goals to zero.
  const noDataXg = calculateExpectedGoals(emptyTeamStats, emptyTeamStats, league);
  assert.equal(noDataXg.homeAttackStrength, 1, "zero games played should default attack strength to league average");
  assert.ok(noDataXg.homeXg > 0, "expected goals should never collapse to zero purely from missing data");

  // --- calculatePoissonOutcomeProbabilities ---
  const evenMatch = calculatePoissonOutcomeProbabilities(1.4, 1.4);
  assert.ok(Math.abs(evenMatch.home + evenMatch.draw + evenMatch.away - 100) < 0.01, "outcome probabilities must always sum to 100");
  assert.ok(Math.abs(evenMatch.home - evenMatch.away) < 1, "identical home/away xG should produce a near-symmetric split");

  const lopsidedMatch = calculatePoissonOutcomeProbabilities(3.2, 0.5);
  assert.ok(Math.abs(lopsidedMatch.home + lopsidedMatch.draw + lopsidedMatch.away - 100) < 0.01, "lopsided outcome probabilities must still sum to 100");
  assert.ok(lopsidedMatch.home > lopsidedMatch.away, "a much higher home xG should favour a home win");
  assert.ok(lopsidedMatch.mostLikelyScoreline.home >= lopsidedMatch.mostLikelyScoreline.away, "most likely scoreline should lean home for a heavily home-favoured match");

  // --- calculateFixturePoissonProbabilities (end-to-end) ---
  const endToEnd = calculateFixturePoissonProbabilities(leagueFixtures[0], leagueFixtures, -0.1);
  assert.ok(Math.abs(endToEnd.home + endToEnd.draw + endToEnd.away - 100) < 0.01, "end-to-end fixture probabilities must sum to 100");
  assert.equal(endToEnd.league.source, "league-data", "end-to-end helper should use the real league averages computed from the fixture list");
}
