...
export function parseFixtureAdvancedEvidence(record: AdvancedEvidenceRecord): FixtureAdvancedEvidence | undefined {
  const evidence: FixtureAdvancedEvidence = {
    ...
    match: {
      ...
      venueCity: firstText(record, ["venue_city"]) || undefined,
      venueCountry: firstText(record, ["venue_country"]) || undefined,
      kickoffUtc: firstText(record, ["kickoff_utc"]) || undefined,
      weatherDetail: firstText(record, ["weather_detail"]) || undefined,
...
export function advancedEvidenceToFixtureRow(evidence: FixtureAdvancedEvidence | undefined): Record<string, string | number | boolean> {
  ...
  return {
    ...
    venue_country: match?.venueCountry ?? "",
    kickoff_utc: match?.kickoffUtc ?? "",
    weather_detail: match?.weatherDetail ?? "",
